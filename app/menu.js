...
function edit_menu_template(win, chat) {
	return {
		label: '&Edit',
		submenu: [
			chat ? {label: 'Undo', accelerator: 'Cmd+Z', role: 'undo'} : {label: 'Undo', id: 'undo', accelerator: darwin ? 'Cmd+Z' : 'Ctrl+Z', click(item) {win.send('undo');}, enabled: false},
			chat ? {label: 'Redo', accelerator: 'Cmd+Shift+Z', role: 'redo'} : {label: 'Redo', id: 'redo', accelerator: darwin ? 'Cmd+Shift+Z' : 'Ctrl+Shift+Z', click(item) {win.send('redo');}, enabled: false},
			{type: 'separator'},
